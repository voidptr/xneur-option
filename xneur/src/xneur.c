...
static void xneur_load_config(void)
{
	...
	log_message(LOG, _("Flush internal buffer when pressed Escape mode set to %s"), xconfig->get_bool_name(xconfig->flush_buffer_when_press_escape));
	...
	log_message(LOG, _("Tracking input mode set to %s"), xconfig->get_bool_name(xconfig->tracking_input));
